...
class CosmetologyBlock
{
    ...
    public function __construct(AtlasReport $pdf)
    {
        ...
        if ($this->pdf->isSectionNeeded('cosmetology')) {
        ...
        $this->pdf->addon->set('cosmetology');
    }
    ...
    private function setTemplate(): void
    {
        ...
        $this->template = [
            'cover-cosmetology' => 'bg/cosmo.jpg',
        ];

        if ('emc' === $template_name) {
            $this->template = [
                'cover-cosmetology' => 'bg/emc/emc_section.jpg',
            ];
        }
    }
    ...
    {
        //block cover page
        $this->pdf->serviceLocator->getBookmarks()->addBookmark('cosmetology');
        $this->pdf->cover->set($this->template['cover-cosmetology']);
        $this->pdf->coverTitle->set('cosmetology');
//        $this->pdf->coverSubtitle->set('healthDescr');

        //risks
        $this->pdf->serviceLocator->getBookmarks()->addBookmark('cosmetology_risks', 1);

        //risks table
        $risksList = new Risks($this->pdf, 'cosmetology_risks');
        ...
        //traits
        $this->pdf->serviceLocator->getBookmarks()->addBookmark('cosmetology_traits', 1);
    ...
    public function getRisks()
    {
        ...
        foreach ($this->pdf->data['cosmetology']['risks'] as $key => $risk) {
    ...
    public function getTraits()
    {
        ...
        foreach ($this->pdf->data['cosmetology']['nutrition'] as $key => $topic) {
            $traits[] = $topic;
        }
        foreach ($this->pdf->data['cosmetology']['sport'] as $key => $topic) {
            $traits[] = $topic;
        }
        foreach ($this->pdf->data['cosmetology']['traits'] as $key => $topic) {
            $traits[] = $topic;
        }
