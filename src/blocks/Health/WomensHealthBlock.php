...
class WomensHealthBlock
{
    ...
    public function __construct(AtlasReport $pdf)
    {
        ...
        if ($this->pdf->isSectionNeeded('womensHealth')) {
        ...
        $this->pdf->addon->set('womensHealth');
    }
    ...
    private function setTemplate(): void
    {
        ...
        $this->template = [
            'cover-womenshealth' => 'bg/womensHealth.jpg',
        ];

        if ('emc' === $template_name) {
            $this->template = [
                'cover-womenshealth' => 'bg/emc/emc_section.jpg',
            ];
        }
    }
    ...
    {
        //block cover page
        $this->pdf->serviceLocator->getBookmarks()->addBookmark('womenshealth');
        $this->pdf->cover->set($this->template['cover-womenshealth']);
        $this->pdf->coverTitle->set('womenshealth');
    ...
    public function getRisks()
    {
        ...
        foreach ($this->pdf->data['womensHealth']['risks'] as $key => $risk) {
    ...
    public function getTraits()
    {
        ...
        foreach ($this->pdf->data['womensHealth']['nutrition'] as $key => $topic) {
            $traits[] = $topic;
        }
        foreach ($this->pdf->data['womensHealth']['sport'] as $key => $topic) {
            $traits[] = $topic;
        }
        foreach ($this->pdf->data['womensHealth']['traits'] as $key => $topic) {
            $traits[] = $topic;
        }
